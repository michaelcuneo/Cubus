  ...
  public sealed partial class WorldStreamer
  {
    ...
    private BlockChunkNeighborhood CreateBlockMeshNeighborhood(Vector3Int root, BlockChunkData center)
    {
      ...
      return new BlockChunkNeighborhood(
        ...
        zPositive,
        zNegative,
        xPositive == null && IsInBoundsBlockNeighbor(root + Vector3Int.right),
        xNegative == null && IsInBoundsBlockNeighbor(root + Vector3Int.left),
        yPositive == null && IsInBoundsBlockNeighbor(root + Vector3Int.up),
        yNegative == null && IsInBoundsBlockNeighbor(root + Vector3Int.down),
        zPositive == null && IsInBoundsBlockNeighbor(root + new Vector3Int(0, 0, 1)),
        zNegative == null && IsInBoundsBlockNeighbor(root + new Vector3Int(0, 0, -1))
      );
    }
    ...
    private void RequeueSettledBlockNeighbors(Vector3Int chunkCoord)
    {
      for (int i = 0; i < BlockMeshNeighborOffsets.Length; i++)
      {
        ...
        if (pendingBlockRenderSet.Contains(n) || pendingBlockRenderRetrySet.Contains(n))
        {
          continue;
        }
